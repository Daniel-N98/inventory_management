...
import { Home, Package, ListChecks, Settings, Users } from "lucide-react";
...
export default function SideNav() {
  ...
  return (
    <aside className="w-full md:w-64 border-b md:border-b-0 md:border-r border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-6 flex flex-col">
      <h2 className="text-2xl font-bold mb-8 text-zinc-900 dark:text-zinc-50">Inventory</h2>
      <nav className="flex flex-row md:flex-col gap-2 md:gap-3 overflow-x-auto md:overflow-visible">
        {navItems.map((item) => (
          <Button
            key={item.label}
            variant="ghost"
